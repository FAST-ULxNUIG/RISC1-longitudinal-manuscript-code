# Script to source all necceary functions for the simulation: -------------
library(fda) # CRAN v5.5.1
source(here::here("code", "functions", "add_poly_to_df.R"))
source(here::here("code", "functions", "add_natural_splines_to_df.R"))
source(here::here("code", "functions", "construct_fd_from_scores.R"))
source(here::here("code", "functions", "decenter_fd_around_new_mean.R"))
source(here::here("code", "functions", "pca.fd_pve_cutoff.R"))
source(here::here("code", "functions", "add_pca.fd_scores_to_df.R"))
source(here::here("code", "functions", "fit_poly.R"))
source(here::here("code", "functions", "fit_naive.R"))
source(here::here("code", "functions", "fit_spline.R"))
source(here::here("code", "functions", "fit_fpca.R"))
source(here::here("code", "functions", "fit_spline_subject_ri_side.R"))
source(here::here("code", "functions", "fit_spline_subject_rs_side.R"))
source(here::here("code", "functions", "fit_naive_spline_intercept.R"))
source(here::here("code", "functions", "split_train_test.R"))
source(here::here("code", "functions", "calculate_prediction_error.R"))
source(here::here("code", "functions", "calculate_rate_of_change.R"))
source(here::here("code", "functions", "calculate_individual_prediction_errors.R"))
source(here::here("code", "functions", "functions-helper-smoothing.R"))
source(here::here("code", "functions", "extract_fixef_coef.R"))
source(here::here("code", "functions", "post-processing-functions.R"))
source(here::here("code", "functions", "theme_gunning.R"))