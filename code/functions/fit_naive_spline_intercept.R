...
fit_naive_spline_intercept <- function(df_scores,
                     K_retain,
                     df, 
  ...
  # Step 2 - Set up formula for model fitting:
  ranef_formula <- "(1 | subject_id/side)"
...
predict_scores_naive_spline_intercept <- function(fit_naive_spline_intercept_object, newdata) {
  # Step 1 -- Unpack objects
  spline_basis <- fit_naive_spline_intercept_object$spline_basis
  lme_fit_list <- fit_naive_spline_intercept_object$lme_fit_list
  K_retain <- fit_naive_spline_intercept_object$K_retain
  # routine checks:
...
predict_fd_naive_spline_intercept <- function(fit_naive_spline_intercept_object, newdata, pca_fd_obj) {
  
  # Extract K from naive fit object:
  K_retain <- fit_naive_spline_intercept_object$K_retain
  
  # Predict scores:
  scores_predictions_matrix <- predict_scores_naive_spline_intercept(fit_naive_spline_intercept_object, newdata)
...
extract_intercept_fd_naive_spline_intercept <- function(fit_naive_spline_intercept_object, pca_fd_obj) {
  ...
  spline_basis_coefficients <- extract_fixef_coef(lme_list_object = fit_naive_spline_intercept_object$lme_fit_list,
                                                K_retain = fit_naive_spline_intercept_object$K_retain)
  ...
  intercept_on_longitudinal_grid <- construct_fd_from_scores(pca_fd_obj = pca_fd_obj,
                                                             ...
                                                             K = fit_naive_spline_intercept_object$K_retain, 
                                                             add_back_mean = TRUE)
