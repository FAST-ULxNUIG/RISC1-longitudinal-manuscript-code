...
fit_spline_subject_rs_side <- function(df_scores,
                                       K_retain,
                                       df, 
  ...
  if(!all(c(paste0("score_", seq_len(K_retain)), "time") %in% names(df_scores))) {
  ...
  if(!diagonal_covariance) {
    ranef_formula <- paste0("(", spline_formula, " | subject_id) + (1 + time|subject_id:side)")
