...
for(i in 1:10) {
  print(lme_test_list[[2]][[i]]@optinfo)
}


lapply(seq_len(k), FUN = function(k) {
  formula_k <- formula(paste0("score_", k, "~ poly_1 + poly_2 + sex + speed_cent +
                               (poly_1 + poly_2||subject_id) + 
                               (poly_1 + poly_2||subject_id:side)"))
  lmer(formula = formula_k, data = test_add_scores, control= lmerControl(optCtrl = list(xtol_rel=0,
...
singular <- (sapply(lme_test_list, function(y) {lapply(y, function(x) x@optinfo$conv$lme4$messages)}))
(non_singular_fit_pc <- apply(singular, 1, function(x) {mean(sapply(x, is.null))}))
# all look ok except for score 7!
