...
predict_scores_naive <- function(fit_naive_object, newdata) {
  # Step 1 -- Unpack objects
  poly_basis <- fit_naive_object$poly_basis
  lme_fit_list <- fit_naive_object$lme_fit_list
...
predict_fd_naive <- function(fit_naive_object, newdata, pca_fd_obj) {
  
  # Extract K from naive fit object:
  K_retain <- fit_naive_object$K_retain
  
  # Predict scores:
  scores_predictions_matrix <- predict_scores_naive(fit_naive_object, newdata)
